    ...
    public class GraphicData
    {
        ...
        public static void Apply(List<GraphicData> data)
        {
            ...
            foreach (var panelData in data)
            {
                ...
                {
                    var layer = panel.GetLayer(layerData.depht, createIfDoesNotExist: true);
                    if (layer.currentGraphic == null || layer.currentGraphic.graphicName != layerData.graphicName)
                    {
                        if (!layerData.isVideo)
                        {
                            Texture tex = HistoryCash.LoadImage(layerData.graphicPath);
                            if (tex != null)
                        ...
                        {
                            VideoClip clip = HistoryCash.LoadVideo(layerData.graphicPath);
                            if (clip != null)
