...
namespace DIALOGUE
{
    public class AutoReader : MonoBehaviour
    {
        ...
        private const float READ_TIME_PADING = 0.5f;
        private const float MAX_READ_TIME = 99;
        ...
        private IEnumerator AutoRead()
        {
            ...
            {
                DialogueSystem.instance.OnSystemPromt_Next();
            }
            ...
            while (conversationManager.isRunning)
            {
                if (!skip)
                {
                    ...
                    timeToRead = (timeToRead / speed) + READ_TIME_PADING;
