...
namespace CHARACTERS
{
    public abstract class Character
    {
        ...
        private const float UNHIGHLITED_DARKEN_STRENGTH = 0.65f;
        ...
        protected Color displayColor => highlighted ? hilightedColor : unhilightedColor; 
        protected Color hilightedColor => color;
        protected Color unhilightedColor =>new Color(color.r * UNHIGHLITED_DARKEN_STRENGTH, color.g * UNHIGHLITED_DARKEN_STRENGTH, color.b * UNHIGHLITED_DARKEN_STRENGTH, color.a);
        ...
        public bool isHighliting => (highlighted && co_highlighting != null);
        public bool isUnHighliting => (!highlighted && co_highlighting != null);
        ...
        public virtual void SetPosition(Vector2 position)
        {
            ...
            (Vector2 minAnhorTarget, Vector2 maxAnhorTarget) = ConvertUITargetPositionToRelativeCharacterAnchorTargets(position);

            root.anchorMin = minAnhorTarget;
            root.anchorMax = maxAnhorTarget;
        ...
        private IEnumerator MovingToPosition(Vector2 position,float speed,bool smooth = false)
        {
            (Vector2 minAnhorTarget, Vector2 maxAnhorTarget) = ConvertUITargetPositionToRelativeCharacterAnchorTargets(position);
            Vector2 padding = root.anchorMax - root.anchorMin;

            while (root.anchorMin != minAnhorTarget || root.anchorMax != maxAnhorTarget)
            {
                root.anchorMin = smooth ?
                    Vector2.Lerp(root.anchorMin, minAnhorTarget, speed * Time.deltaTime)
                    : Vector2.MoveTowards(root.anchorMin, minAnhorTarget, speed * Time.deltaTime * 0.35f);

                root.anchorMax = root.anchorMin + padding;

                if (smooth && Vector2.Distance(root.anchorMin, minAnhorTarget) <= 0.001f)
                {
                    root.anchorMin = minAnhorTarget;
                    root.anchorMax = maxAnhorTarget;
                    break;
        ...
        {
            Debug.Log("Color changing is not applicable on this caracter type");
            yield return null;
        }

        public Coroutine Highlight(float speed = 1f,bool immediate = false)
        {
            if (isHighliting || isUnHighliting)
            {
        ...
        {
            Debug.Log("Hilighting is not avalaible on this character type!");
