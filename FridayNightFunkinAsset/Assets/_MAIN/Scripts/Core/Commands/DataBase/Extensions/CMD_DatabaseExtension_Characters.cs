...
namespace COMMANDS
{
    ...
    {
        private static string[] PARAM_ENABLED => new string[] { "-e", "-enable" };
        ...
        public static IEnumerator HighlightAll(string[] data)
        {
            ...
            parameters.TryGetValue(new string[] { "-i", "-immediate" }, out immediate, defaultValue: false);
            parameters.TryGetValue(new string[] { "-o", "-only" }, out handleUnspecifiedCharacters, defaultValue: false);
        ...
        public static IEnumerator UnHighlightAll(string[] data)
        {
            ...
            if (!immediate)
            {
                ...
                while (characters.Any(c => c.isUnHighliting) || (handleUnspecifiedCharacters && unspecifiedCharacters.Any(uc => uc.isHighliting)))
                    yield return null;
            }
        }
