...
public class VariableStore
{
    ...
    private static Dictionary<string, Databse> databases = new Dictionary<string, Databse>() { { DEFAULT_DATABASE_NAME, new Databse(DEFAULT_DATABASE_NAME)} };
    private static Databse defaultDatabase => databases[DEFAULT_DATABASE_NAME];

    public static bool CreateDatabase(string name)
    {
        if (!databases.ContainsKey(name))
        {
            databases[name] = new Databse(name);
            return true;
        }
    ...
    public static Databse GetDatabse(string name)
    {
    ...
    {
        (string[] parts, Databse db, string variableName) = ExtractInfo(name);
    ...
    private static (string[], Databse, string) ExtractInfo(string name)
    {
        string[] parts = name.Split(DATABASE_VARIABLE_RELATIONAL_ID);
        Databse db = parts.Length > 1 ? GetDatabse(parts[0]) : defaultDatabase;
    ...
    {
        foreach (KeyValuePair<string,Databse> dbEntry in databases)
        {
    ...
    {
        databases.Clear();
        databases[DEFAULT_DATABASE_NAME] = new Databse(DEFAULT_DATABASE_NAME);
    }

    public static void PrintAllVariables(Databse database = null)
    {
        if (database != null)
        {
    ...
    private static void PrintAllDatabaseVariables(Databse database)
    {
        StringBuilder sb = new StringBuilder();
        sb.AppendLine($"Database: <color=#F38544>{database.name}</color>");

        foreach (KeyValuePair<string,Variable> valuePair in database.variables)
        {
            string variableName = valuePair.Key;
            object variableValue = valuePair.Value.Get();
